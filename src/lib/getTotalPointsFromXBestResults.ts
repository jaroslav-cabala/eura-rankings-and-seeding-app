import { RankedPlayerTournamentResultDTO } from "@/api/apiTypes";
 ...
 * @returns total points of X best results or 0 if 'results' parameter is undefined or an empty array
 */
export const getTotalPointsFromXBestResults = (
  ...
  return results?.reduce((acc, curr) => (acc += curr.points), 0) ?? 0;
};

type Results = Pick<
  RankedPlayerTournamentResultDTO,
  "points" | "date" | "tournamentId" | "tournamentResultId"
>;
